...
def func_(data):
    ...
    # score = re.search(r'\d+', store.content).group()
    # Changes, updatedJD = store.content.split("Updated Job Description:")
    # store_content, updatedJD, score = func_(data=data)
    # return store_content, updatedJD, score
    return store
...
def divisor(store):
    score=re.search(r'\d+', store)
    Changes, updatedJD = (store.split("Updated Job Description:"))
    return([score.group(), updatedJD, Changes])
    # print(store.content)
    # print(updatedJD)
    # print("-"*30)
    # print (score.group())
